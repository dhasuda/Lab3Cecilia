    ...
    for (int i=0; i<particles.size(); i++) {
        ...
        for (int k = 0; k < lowerBound.size(); k++) {
            velocity.push_back(w * particles[i].getVelocity()[k] +
                               c1 * (double) rand()/RAND_MAX * (particles[i].getPersonalBestPosition()[k] - particles[i].getPosition()[k]) +
                                c2 * (double) rand()/RAND_MAX * (globalBestPosition[k] - particles[i].getPosition()[k]));
